...
void setup(void) {
  ...
  //lua_close(L);
  /*
  for(uint16_t y=0; y < 64-1;y++){
      for(uint16_t x=0; x < 64-1;x++){
        tft.drawPixel(200+x,64+y, 0x543F);
      }
    }

    tft.drawPixel(32,72, 0x543F);
    tft.drawPixel(33,73, 0x543F);
    tft.drawPixel(34,74, 0x543F);
  delay(16000);
  */
...
/*
int lua_drawDrawable(lua_State* L)
{
	...
	//create the drawable and push into the map
	drawables[name]->draw(orthoPersp, orthoViewMatrix, vector2(x, y));
	updateAnimation(drawables[name]->_graphicsObject);
